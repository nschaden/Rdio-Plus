...
var ExtraControls = function($)
{
	...
	function keyCheck(event)
	{
		...
		// for Cmd + Shift + I, add currently playing song to Inbox playlist
		if (event.shiftKey && event.metaKey && event.keyCode == 105)
		{
			if (typeof QuickAdd == 'object' && typeof QuickAdd.addCurrentlyPlayingTrackToPlaylistName == 'function')
			{
			    event.preventDefault();
			    QuickAdd.addCurrentlyPlayingTrackToPlaylistName('Inbox');
		    }
		}
		
		// for Cmd + Shift + J, run Echonest search based on current artist
		if (event.shiftKey && event.metaKey && event.keyCode == 106)
		{
		    if (typeof QuickAdd == 'object' && typeof QuickAdd.showEchonestDialog == 'function')
			{
			    event.preventDefault();
			    QuickAdd.showEchonestDialog(false);
		    }
		}
		
		// for Cmd + Shift + K, run Echonest search based on current song
		if (event.shiftKey && event.metaKey && event.keyCode == 107)
		{
		    if (typeof QuickAdd == 'object' && typeof QuickAdd.showEchonestDialog == 'function')
			{
			    event.preventDefault();
			    QuickAdd.showEchonestDialog(true);
		    }
		}
...
var QuickAdd = function($)
{
	...
	var limitEchonestRdioUS = true;
	var statusTimer;
	var nowplayingDialogInit = false;
	var overlayScreen;
	var echonestDataQueue;
	var echonestQueriedData;
	var playlistDialog;
    ...
    function addCurrentlyPlayingTrackToPlaylistName(name,description)
    {
        var currentlyplayinghref = $('#playerNowPlayingTitle a').attr('href');
        if (typeof currentlyplayinghref != 'string' || currentlyplayinghref.length == 0) return false;
        $.getJSON(currentlyplayinghref,function(returndata)
        { 
            if (typeof returndata != 'object') return false;
            var returntrackid = returndata.content.content.data.track.id;
            if (typeof returntrackid != 'number') return false;
            QuickAdd.currentTrackids = [returntrackid];
            QuickAdd.addTracksFromExistingTracklistToPlaylistName(name,description);
            return true;
        });
        return true;
    }
	...
	function addTracksFromExistingTracklistToPlaylist(playlist)
	{
		if (typeof playlist == 'undefined') return false;
		QuickAdd.currentPlaylist = playlist;

		QuickAdd.addIndicatorMessage('Adding to playlist ' + playlist.name + '...');
		QuickAdd.currentTrackIndex = 0;
		QuickAdd.addToPlayListLoop();
		return true;
	}
	
	function addTracksFromExistingTracklistToPlaylistName(name,description)
	{
	    var returnlist;
		getPlaylist(name || 'Quick',function(data) 
		{ 
			...
			if (typeof returnlist != 'object')
			{
				...
				{
					if (data2)
						addTracksFromExistingTracklistToPlaylistName(name || 'Quick');
				});
			}
			else
				addTracksFromExistingTracklistToPlaylist(returnlist);
		});
	}
	...
	{
		return $('.artist_header .name,.mini_header').text().trim();
	}
	...
	// get the title of the song on the current page that has its info tray exposed,
	// by default on pages with only one track, can be invoked by hitting the 'i' button
	// to the right of any track in a song list
	function getCurrentSongTitle()
	{
		return $('#content .song_list .track.info_exposed .title_info').text().trim();
	}
	
	function getEchonestPlaylistFromArtistName(artist,options,callback)
	{
		if (typeof options != 'object' || !options)
			options = {};
		if (typeof options.mood == 'string' && options.mood.trim().length == 0)
		    delete options.mood;
		if (typeof options.variety == 'string' && options.variety.trim().length == 0)
		    delete options.variety;
		if (typeof options.song_min_hotttnesss == 'string' && options.song_min_hotttnesss.trim() == "0" &&
			typeof options.song_max_hotttnesss == 'string' && options.song_max_hotttnesss.trim() == "1")
		{
			delete options.song_min_hotttnesss;
			delete options.song_max_hotttnesss;
		} 
		var baseurl = 'http://developer.echonest.com/api/v4/playlist/static?api_key=PWNYOEPUNHQTGZLHY&format=json&type=artist-radio';
		if (limitEchonestRdioUS)
			baseurl = 'http://developer.echonest.com/api/v4/playlist/static?api_key=PWNYOEPUNHQTGZLHY&format=json&bucket=id:rdio-us-streaming&limit=true&type=artist-radio';
		options.artist = artist;
		$.getJSON(baseurl,options,callback);
		return true;
	}
	
	function getEchonestPlaylistFromSongTitleAndArtist(song,artist,options,callback)
	{
		var songsearchurl = 'http://developer.echonest.com/api/v4/song/search?api_key=PWNYOEPUNHQTGZLHY&format=json&results=1';
		$.getJSON(songsearchurl,{artist: artist, title: song},function(returndata)
		{
			if (typeof options != 'object' || !options)
				options = {};
			if (typeof options.mood == 'string' && options.mood.trim().length == 0)
			...
			if (returndata.response.songs.length != 1) return false;
			var songid = returndata.response.songs[0].id;
			if (typeof songid != 'string') return false;
			
			var baseurl = 'http://developer.echonest.com/api/v4/playlist/static?api_key=PWNYOEPUNHQTGZLHY&format=json&type=song-radio';
			if (limitEchonestRdioUS)
				baseurl = 'http://developer.echonest.com/api/v4/playlist/static?api_key=PWNYOEPUNHQTGZLHY&format=json&bucket=id:rdio-us-streaming&limit=true&type=song-radio';
				
			options.song_id = songid;
			$.getJSON(baseurl,options,callback);
			return true;
		});
		return true;
	...
	function runMultiSearchQueryFromEchonest()
	{
		QuickAdd.echonestQueriedData = [];
		if (QuickAdd.echonestDataQueue.length == 0) 
		{
		    // done 
		    QuickAdd.showPlaylistDialog('Add Echonest results to','existinglist');
		    return true;
		}
		var querystr = "";
		var currsong = QuickAdd.echonestDataQueue.shift();
		QuickAdd.echonestQueriedData.push(currsong);
		querystr += currsong.title + " " + currsong.artist_name;

		$.getJSON('/api/json/search/', {query: querystr}, function(data)
		{ 
			// compare return data vs. queried data
			
			for (var i = 0; i < QuickAdd.echonestQueriedData.length; i++)
			{
				var echonestdata = QuickAdd.echonestQueriedData[i];
				for (var j = 0; j < data.length; j++)
				{
				    var rdiodata = data[j];
				    // consider only returned, streamable tracks from rdio
					if (rdiodata.type == "t" && rdiodata.can_stream &&
					    rdiodata.artist.trim().toLowerCase() == echonestdata.artist_name.trim().toLowerCase() &&
						rdiodata.name.trim().toLowerCase() == echonestdata.title.trim().toLowerCase())
					{
						if ($.inArray(rdiodata.id,QuickAdd.currentTrackids) == -1)
						    QuickAdd.currentTrackids.push(rdiodata.id);
						break;
					}
				}
			}
			runMultiSearchQueryFromEchonest();
		});
		return true;
	}
	
	function showEchonestDialog(songmode)
	{
		var song,artist;
		if (songmode)
		{
			song = getCurrentSongTitle();
			artist = getCurrentArtistName(); 
			if (song.length == 0 || artist.length == 0) return false;
		}
		else
		{
			artist = getCurrentArtistName(); 
		...
		var entryvariety = $('<div class="echo_option" style="height: 26px; padding: 5px;"><label for="variety" style="width: 110px; padding-top: 5px; text-align: right; float: left; clear: left;">Variety:</label><div class="editable" style="display: none; position: relative; float: right;">(0 to 1) <input type="text" name="variety" class="textInput" style="width: 20px;" value=""></div><div class="default" style="position: relative; float: right; clear: right; padding-top: 5px;">Default</div></div>');
		var entrydance = $('<div class="echo_option" style="height: 26px; padding: 5px;"><label for="dance" style="width: 110px; padding-top: 5px; text-align: right; float: left; clear: left;">Danceability:</label><div class="editable" style="display: none; position: relative; float: right;"><input type="text" name="min_danceability" class="textInput" style="width: 20px;" value="0"> to <input type="text" name="max_danceability" class="textInput" style="width: 20px;" value="1"></div><div class="default" style="position: relative; float: right; clear: right; padding-top: 5px;">Default</div></div>');
		var entryenergy = $('<div class="echo_option" style="height: 26px; padding: 5px;"><label for="energy" style="width: 110px; padding-top: 5px; text-align: right; float: left; clear: left;">Energy:</label><div class="editable" style="display: none; position: relative; float: right;"><input type="text" name="min_energy" class="textInput" style="width: 20px;" value="0"> to <input type="text" name="max_energy" style="width: 20px;" value="1" class="textInput"></div><div class="default" style="position: relative; float: right; clear: right; padding-top: 5px;">Default</div></div>');
		var entryfamiliarity = $('<div class="echo_option" style="height: 26px; padding: 5px;"><label for="artist_familiarity" style="width: 110px; padding-top: 5px; text-align: right; float: left; clear: left;">Artist Familiarity:</label><div class="editable" style="display: none; position: relative; float: right;"><input type="text" name="artist_min_familiarity" class="textInput" style="width: 20px;" value="0"> to <input type="text" name="artist_max_familiarity" style="width: 20px;" value="1" class="textInput"></div><div class="default" style="position: relative; float: right; clear: right; padding-top: 5px;">Default</div></div>');
		var entryhotness = $('<div class="echo_option" style="height: 26px; padding: 5px;"><label for="artist_hotness" style="width: 110px; padding-top: 5px; text-align: right; float: left; clear: left;">Artist Hotness:</label><div class="editable" style="display: none; position: relative; float: right;"><input type="text" name="artist_min_hotttnesss" class="textInput" style="width: 20px;" value="0"> to <input type="text" name="artist_max_hotttnesss" style="width: 20px;" value="1" class="textInput"></div><div class="default" style="position: relative; float: right; clear: right; padding-top: 5px;">Default</div></div>');
		var entrysonghotness = $('<div class="echo_option" style="height: 26px; padding: 5px;"><label for="song_hotness" style="width: 110px; padding-top: 5px; text-align: right; float: left; clear: left;">Song Hotness:</label><div class="editable" style="display: none; position: relative; float: right;"><input type="text" name="song_min_hotttnesss" class="textInput" style="width: 20px;" value="0"> to <input type="text" name="song_max_hotttnesss" style="width: 20px;" value="1" class="textInput"></div><div class="default" style="position: relative; float: right; clear: right; padding-top: 5px;">Default</div></div>');
		var entrymood = $('<div class="echo_option" style="height: 26px; padding: 5px;"><label for="mood" style="width: 110px; padding-top: 5px; text-align: right; float: left; clear: left;">Mood:</label><div class="editable" style="display: none; position: relative; float: right;"><input type="text" name="mood" class="textInput" style="width: 160px;" value=""></div><div class="default" style="position: relative; float: right; clear: right; padding-top: 5px;">Default</div></div>');
		var entryresults = $('<div class="echo_option" style="height: 26px; padding: 5px;"><label for="results" style="width: 110px; padding-top: 5px; text-align: right; float: left; clear: left;">Results:</label><div class="editable" style="display: none; position: relative; float: right;"><input type="text" name="results" class="textInput" style="width: 20px;" value="25"></div><div class="default" style="position: relative; float: right; clear: right; padding-top: 5px;">Default</div></div>');
		
		var clearfix = $('<div style="clear: both; height: 1px;"></div>');
		var fulloptions = $('<form id="echonest_options" style="background: #fff; padding:16px;"></form>').append(entryvariety,entrydance,entryenergy,entryfamiliarity,entryhotness,entrysonghotness,entrymood,entryresults,clearfix);
		var addtoplaylist = QuickAdd.playlistDialog.find('#add_to_playlist_dialog').replaceWith(fulloptions);
		
		if (songmode)
			QuickAdd.showPlaylistDialog('Run Echonest search for song ' + song);
		else
			QuickAdd.showPlaylistDialog('Run Echonest search for artist ' + artist);
		
		// add functionality for confirm/cancel buttons
		QuickAdd.playlistDialog.find('.ui-dialog-buttonset .cancel').click(function()
		{
			QuickAdd.playlistDialog.find('#echonest_options').replaceWith(addtoplaylist);
			QuickAdd.playlistDialog.find('.ui-dialog-buttonset .add').text('Add');
		});
		QuickAdd.playlistDialog.find('.ui-dialog-titlebar-close').click(function()
		{
			QuickAdd.playlistDialog.find('#echonest_options').replaceWith(addtoplaylist);
			QuickAdd.playlistDialog.find('.ui-dialog-buttonset .add').text('Add');
		});	
		QuickAdd.playlistDialog.find('.ui-dialog-buttonpane .add').unbind();
		QuickAdd.playlistDialog.find('.ui-dialog-buttonset .add').text('Run').click(function()
		{
			var serialized = $('#echonest_options').serializeArray();
			var options = {};
			for (var i = 0; i < serialized.length; i++)
			{
			    options[serialized[i].name] = serialized[i].value;
			}
			QuickAdd.playlistDialog.remove();
			if (songmode)
				QuickAdd.getEchonestPlaylistFromSongTitleAndArtist(song,artist,options,function(returndata) 
				{ 
					QuickAdd.playlistDialog.find('#echonest_options').replaceWith(addtoplaylist);
					QuickAdd.playlistDialog.find('.ui-dialog-buttonset .add').text('Add');
					QuickAdd.echonestDataQueue = returndata.response.songs;
					QuickAdd.currentTrackids = [];
					if (!limitEchonestRdioUS)
						runMultiSearchQueryFromEchonest();
					else
					{
						for (var i = 0; i < returndata.response.songs.length; i++)
						{
							if (typeof returndata.response.songs[i].foreign_ids == 'object' && returndata.response.songs[i].foreign_ids.length > 0)
							{
								var trackid = returndata.response.songs[i].foreign_ids[0].foreign_id;
								var parsednum = /song:t(\d*)/.exec(trackid);
								if (parsednum != null && parsednum.length > 1)
									QuickAdd.currentTrackids.push(parseInt(parsednum[1],10));
							}
						}
						QuickAdd.showPlaylistDialog('Add Echonest results to','existinglist');
					}
				...
				QuickAdd.getEchonestPlaylistFromArtistName(artist,options,function(returndata) 
				{ 
					QuickAdd.playlistDialog.find('#echonest_options').replaceWith(addtoplaylist);
					QuickAdd.playlistDialog.find('.ui-dialog-buttonset .add').text('Add');
					QuickAdd.echonestDataQueue = returndata.response.songs;
		...
		{
			$(this).addClass('editing').children('.default').fadeOut(function() { $(this).siblings('.editable').fadeIn(); });
		});
		return true;
	}
	...
	function showPlaylistDialog(dialogtitle,dialogmode)
	{
		...
		{ 
			...
			if (!$('#My_Playlists').hasClass('ui-tabs-hide'))
			{
				...
				else if (playlistDialogMode == 'existinglist')
				    addTracksFromExistingTracklistToPlaylistName($('#playlistSelection')[0].value);
			}
			...
			else
			{
				if (playlistDialogMode == 'artist')
				    ...
				    addTracksFromExistingTracklistToPlaylistName($('#playlistName')[0].value.trim(),$('#playlistDescription')[0].value.trim());
			}
	...
	exports.addCurrentlyPlayingTrackToPlaylistName = addCurrentlyPlayingTrackToPlaylistName;
	...
	exports.addTracksFromExistingTracklistToPlaylist = addTracksFromExistingTracklistToPlaylist;
	exports.addTracksFromExistingTracklistToPlaylistName = addTracksFromExistingTracklistToPlaylistName;
	...
	exports.getCurrentArtistName = getCurrentArtistName;
	exports.getCurrentArtistURL = getCurrentArtistURL;
	exports.getCurrentSongTitle = getCurrentSongTitle;
	exports.getEchonestPlaylistFromArtistName = getEchonestPlaylistFromArtistName;
	exports.getEchonestPlaylistFromSongTitleAndArtist = getEchonestPlaylistFromSongTitleAndArtist;
	...
	exports.runMultiSearchQueryFromEchonest = runMultiSearchQueryFromEchonest;
	exports.showEchonestDialog = showEchonestDialog;
...
(function() {
	...
	var helpscreen = $('<div id="extracontrols_help" style="display: none; width: 100%; height: 110%; position: absolute; background: rgba(0,0,0,0.7); top: 0; left: 0; z-index: 999999;"><h2 style="font-size: 1.2em; margin-bottom: 0.5em; padding: 5px;">Keyboard Shortcuts</h2><table><tr><td>Left</td><td>Previous Track</td></tr><tr><td>Right</td><td>Next Track</td></tr><tr><td>Spacebar</td><td>Play/Pause</td></tr><tr><td>Cmd+U</td><td>Now Playing</td></tr><tr><td>Cmd+Shift+C</td><td>Collection</td></tr><tr><td>Cmd+Shift+F</td><td>Full Screen</td></tr><tr><td>Cmd+Shift+H</td><td>Home (Dashboard)</td></tr><tr><td>Cmd+Shift+I</td><td>Song to Inbox</td></tr><tr><td>Cmd+Shift+J</td><td>Echonest artist</td></tr><tr><td>Cmd+Shift+K</td><td>Echonest song</td></tr><tr><tr><td>Cmd+Shift+L</td><td>Search focus</td></tr><tr><td>Cmd+Shift+M</td><td>Queue</td></tr><tr><td>Cmd+Shift+N</td><td>History</td></tr><tr><td>Cmd+Shift+P</td><td>Playlists</td></tr><tr><td>Cmd+Shift+X</td><td>Shuffle toggle</td></tr><tr><td>Cmd+Shift+Z</td><td>Repeat toggle</td></tr><tr><td>Cmd+Shift+,</td><td>Help</td></tr></table></div>');
